"""
data_recorder.py  v2.3 — Edición Mission Brain (Con Lateralidad)
─────────────────────
Igual que v2 + captura automática de mano utilizada (Izquierda/Derecha)
y función de resumen de lateralidad para diagnóstico de asimetría motora.
"""
...
FIELDNAMES = [
    ...
    "hand_used", # <--- NUEVA COLUMNA PARA LATERALIDAD
...
class DataRecorder:
    ...
    def record_trial(self, level, word, word_color, word_color_rgb,
        ...
        # Extracción segura de la mano utilizada desde el tracker
        hand_used = motor_metrics.get("hand_used", "No detectada")
        ...
        row = {
            ...
            "congruent":           congruent,
            "hand_used":           hand_used, # <--- SE GUARDA AQUÍ
    ...
    def get_summary(self):
        """Mantiene compatibilidad con report_generator.py (Agrupa por nivel)"""
        if not self.trials:
            return {}
        summary = {}
        for lvl in sorted(set(t["level"] for t in self.trials)):
            ...
            summary[lvl] = {
                ...
                "std_rt_ms":        round((sum((r - mean_r)**2 for r in rts) / max(len(rts),1)) ** 0.5, 1) if rts else 0,
    ...
    def get_lateralization_summary(self):
        """Genera un diagnóstico diferencial entre la mano izquierda y derecha."""
        lat_summary = {}
        for hand in ["Derecha", "Izquierda"]:
            ht = [t for t in self.trials if t["hand_used"] == hand]
            if not ht:
                continue
            
            ok  = [t for t in ht if t["correct"]]
            rts = [t["rt_ms"] for t in ht if not t["timed_out"]]
            
            lat_summary[hand] = {
                "intentos": len(ht),
                "precision_pct": round(len(ok) / max(len(ht), 1) * 100, 1),
