"""
hand_tracker.py  v2.3 — Edición Mission Brain (Con Lateralidad)
────────────────────
...
- Detección automática de mano utilizada (Izquierda/Derecha) con corrección de espejo.
"""
...
class MotorMetrics:
    def __init__(self):
        ...
        self.hand_used   = "No detectada" # <--- NUEVA PROPIEDAD
    ...
    def reset(self):
        ...
        self.corrections = 0
        self._prev_angle = None
        self.hand_used   = "No detectada"

    def update(self, x, y, hand_label=None): # <--- SE AÑADE HAND_LABEL
        t = time.perf_counter()
        self.trajectory.append((x, y, t))
        
        # Registrar la mano usada (solo si no se había registrado aún o si cambia)
        if hand_label and self.hand_used == "No detectada":
            self.hand_used = hand_label
    ...
    def to_dict(self):
        return {
            ...
            "hand_used":    self.hand_used # <--- EXPORTAR AL DICCIONARIO
        }
...
class HandTracker:
    ...
    def get_frame_and_finger(self):
        ...
        if not ret or frame is None:
            ...
            frame = cv2.flip(frame, 1) # <--- FLIP APLICADO AQUÍ
        ...
        fx, fy = None, None
        hand_label_corrected = None # <--- VARIABLE PARA LA ETIQUETA
        ...
        if res and res.multi_hand_landmarks:
            ...
            # EXTRAER Y CORREGIR LATERALIDAD
            # Al haber hecho flip al frame, lo que MediaPipe ve como 'Left'
            # es en realidad la mano 'Derecha' del paciente.
            if res.multi_handedness:
                raw_label = res.multi_handedness[0].classification[0].label
                hand_label_corrected = "Derecha" if raw_label == "Left" else "Izquierda"
            ...
            if self._active:
                # Pasar la etiqueta de la mano al actualizador de métricas
                self.metrics.update(fx, fy, hand_label_corrected)
